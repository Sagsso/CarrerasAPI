...
module CampeonsService {
    ...
    export async function getByNombre(nombre: string): Promise<any> {
        const query = `SELECT * FROM apicarre_carreras.campeon WHERE piloto = "${nombre}"`
        return await Campeon.execQuery(query);
    }

    export async function create(values: Array<any>): Promise<any> {
        let sql = `INSERT INTO apicarre_carreras.campeon (piloto, titulos, equipo) VALUES ?`;
    ...
    export async function update(id: number, piloto: string, titulos: string, equipo: string): Promise<any> {
        const query = `UPDATE apicarre_carreras.campeon SET piloto = "${piloto}", titulos = "${titulos}", equipo = "${equipo}" WHERE id = ${id}`
        return await Campeon.execQuery(query);
    }
    ...
    export async function addTitulos(id: number, newTitulo: string): Promise<any> {
        ...
        const query1 = `SELECT titulos FROM apicarre_carreras.campeon WHERE id = ${id}`
        ...
        const query2 = `UPDATE apicarre_carreras.campeon SET titulos = "${arrayTitulos}" WHERE id = ${id}`
        return await Campeon.execQuery(query2);
    }


    export async function updateEquipo(id: number,  titulos: string): Promise<any> {
        const query = `UPDATE apicarre_carreras.campeon SET equipo = "${titulos}" WHERE id = ${id}`
        return await Campeon.execQuery(query);
    }
}
